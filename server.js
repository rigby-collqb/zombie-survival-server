...
app.get('/', (_req, res) => {
  res.status(200).json(gameServer ? gameServer.status() : {online:true,version:'26.0.0'});
});
...
app.get('/api/leaderboard', (req, res) => {
  ...
  res.status(200).json({ success:true, rows:gameServer.accounts.leaderboard(limit), version:'26.0.0' });
});

app.get('/api/maps', (_req, res) => {
  res.status(200).json({ success:true, maps:GameServer.MAPS, version:'26.0.0' });
});

app.get('/api/story', (_req, res) => {
  res.status(200).json({ success:true, chapters:StorySystem.CHAPTERS, achievements:Object.values(ACHIEVEMENTS), version:'26.0.0' });
});
