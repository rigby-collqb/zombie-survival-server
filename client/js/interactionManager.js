class InteractionManager {
  ...
  promptFor(i){
    ...
    if(i.type==='medstation')return `USAR ESTAÇÃO MÉDICA · $${i.cost||0}`;
    if(i.type==='ammo_station')return `COMPRAR MUNIÇÃO · $${i.cost||0}`;
  ...
  render(ctx,camera){
    for(const i of this.items.values()){
      ...
      if(i.type==='door'){
        ...
        if(i.state?.opened)ctx.globalAlpha=.45;ctx.fillStyle=i.id.includes('military')?'#44513f':i.id.includes('hospital')?'#ddd8cc':i.id.includes('police')?'#48596a':'#705537';ctx.fillRect(s.x,s.y,i.width,i.height);ctx.strokeStyle='#28251f';ctx.strokeRect(s.x,s.y,i.width,i.height);ctx.beginPath();ctx.moveTo(s.x,s.y);ctx.lineTo(s.x+i.width,s.y+i.height);ctx.moveTo(s.x+i.width,s.y);ctx.lineTo(s.x,s.y+i.height);ctx.stroke();
      }else if(i.type==='medstation'){
        ctx.fillStyle='#d9ddd6';ctx.fillRect(s.x,s.y,i.width,i.height);ctx.fillStyle='#a93636';ctx.fillRect(cx-4,s.y+7,8,i.height-14);ctx.fillRect(s.x+7,cy-4,i.width-14,8);ctx.strokeStyle='#555';ctx.strokeRect(s.x,s.y,i.width,i.height);
      }else if(i.type==='ammo_station'){
        ctx.fillStyle='#4f554a';ctx.fillRect(s.x,s.y,i.width,i.height);ctx.strokeStyle='#b89b55';ctx.strokeRect(s.x,s.y,i.width,i.height);ctx.fillStyle='#d9c178';ctx.fillRect(s.x+8,s.y+9,i.width-16,5);ctx.fillRect(s.x+8,s.y+20,i.width-16,5);
      }
      ctx.restore();
    }
  }
