...
class Player {
  ...
  updateRemotePlayer(s){
    ...
    this.health=s.health??this.health;this.maxHealth=s.maxHealth??this.maxHealth;this.alive=s.alive??this.alive;
    if(typeof s.aimDirX==='number')this.aimDirX=s.aimDirX;if(typeof s.aimDirY==='number')this.aimDirY=s.aimDirY;
    if(s.weaponId)this.currentWeapon=s.weaponId;
    this._renderX=this.x;this._renderY=this.y;
  }
  ...
  renderPlayer(ctx,camera){this._renderAt(ctx,camera,this._renderX,this._renderY,'#3fa9ff');}
  renderRemotePlayer(ctx,camera){this._renderAt(ctx,camera,this._renderX,this._renderY,'#d4c04a');this._renderNameTag(ctx,camera);}

  _weaponStyle(){
    const id=this.currentWeapon;
    if(id==='smg')return{length:24,width:5,color:'#25292b',flash:'#ffe06a'};
    if(id==='shotgun')return{length:30,width:7,color:'#3b3129',flash:'#ffbb45'};
    if(id==='rifle')return{length:34,width:5,color:'#202426',flash:'#fff0a8'};
    return{length:20,width:4,color:'#2a2a2a',flash:'#ffe39a'};
  }

  _renderAt(ctx,camera,worldX,worldY,bodyColor){
    const s=camera.worldToScreen(worldX,worldY),cx=s.x+this.width/2,cy=s.y+this.height/2,dead=this.alive===false;
  ...
  _renderNameTag(ctx,camera){const s=camera.worldToScreen(this._renderX,this._renderY);ctx.fillStyle='rgba(0,0,0,.6)';ctx.font='12px Segoe UI,sans-serif';ctx.textAlign='center';ctx.fillText(this.name,s.x+this.width/2,s.y-10);}
}
