...
class VFXManager{
  constructor(){this.particles=[];this.tracers=[];this.casings=[];this.floatingTexts=[];this.projectiles=[];this.rings=[];this.decals=[];this.lights=[];this.hitMarkerMs=0;this.hitMarkerType='normal';this.shake=0;this.masterVolume=.6;this._audioCtx=null;}
  ...
  addBlood(x,y,amount=14,headshot=false){
    amount=Math.max(1,Math.min(60,Math.round(amount*(headshot?1.45:1))));
  ...
  _addDecal(type,x,y,size){if(this.decals.length>=140)this.decals.splice(0,20);this.decals.push({type,x,y,size,rotation:Math.random()*Math.PI,alpha:type==='blood'?.45:.32});}
  ...
  _addSmoke(x,y,amount=3){for(let i=0;i<amount;i++)this._pushParticle({type:'smoke',x:x+(Math.random()-.5)*6,y:y+(Math.random()-.5)*6,vx:(Math.random()-.5)*24,vy:-12-Math.random()*22,life:.25+Math.random()*.3,maxLife:.55,size:3.5+Math.random()*5.5});}
  _pushParticle(p){if(this.particles.length>=VFX_CONFIG.MAX_PARTICLES)this.particles.splice(0,Math.max(1,Math.floor(VFX_CONFIG.MAX_PARTICLES*.08)));this.particles.push(p);}

  addExplosion(x,y,radius=110){
    this.rings.push({x,y,r:8,maxR:radius,life:.48,maxLife:.48,color:'#ffb13b'});this.addShake(11);
    for(let i=0;i<55;i++){const a=Math.random()*Math.PI*2,s=50+Math.random()*300;this._pushParticle({type:i<30?'spark':'smoke',x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:.3+Math.random()*.75,maxLife:1.05,size:2+Math.random()*7});}
  }
  ...
  update(dt){
    ...
    for(let i=this.particles.length-1;i>=0;i--){const p=this.particles[i];p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;if(p.type==='blood'){p.vy+=175*dt;p.vx*=Math.pow(.2,dt);}else if(p.type==='smoke'){p.vx*=Math.pow(.12,dt);p.vy*=Math.pow(.3,dt);p.size+=8*dt;}else{p.vx*=Math.pow(.1,dt);p.vy*=Math.pow(.1,dt);}if(p.life<=0)this.particles.splice(i,1);}
